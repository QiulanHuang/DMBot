...
with open(output_file, "w", newline="", encoding="utf-8") as flow:
    ...
    for i in range(0, int(total["value"] / 100) + 1):
        ...
        for res in item:
            if (
                type(res["_source"]["dcache"]["billing"]["protocol"]["path"]).__name__
                == "list"
            ):
                # print(res['_source']['dcache']['billing']['protocol']['path'][0])
                csv_writer.writerow(
                    [
                        res["_source"]["@timestamp"]
                        + ","
                        + res["_source"]["event"]["action"]
                        + ","
                        + res["_source"]["dcache"]["billing"]["protocol"]["path"][0]
                        + ","
                        + res["_source"]["message"]
                    ]
                )
                ...
                csv_writer.writerow(
                    [
                        ...
                        + res["_source"]["event"]["action"]
                        + ","
                        + res["_source"]["dcache"]["billing"]["protocol"]["path"]
                        + ","
                        + res["_source"]["message"]
                    ]
                )
